...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Feature veri seti uzerinde LSTM egit.")
    parser.add_argument("--symbols", nargs="*", help="Ornek: --symbols BTC/USD ETH/USD")
    parser.add_argument("--all", action="store_true", help="Tum tanimli varliklar icin LSTM egit.")
    ...
    parser.add_argument("--num-layers", type=int, default=1, help="LSTM katman sayisi")
...
def train_for_symbol(symbol: str, args: argparse.Namespace) -> None:
    ...
    experiment_model_name = resolve_experiment_model_name("lstm", args.class_weight)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        LOGGER.info(
            "LSTM epoch tamamlandi | symbol=%s | epoch=%s/%s | train_loss=%.4f | val_loss=%.4f | val_f1=%.4f | val_roc_auc=%s",
            symbol,
            epoch,
            args.epochs,
            train_loss,
    ...
    if best_state is None:
        raise RuntimeError("LSTM egitimi sirasinda en iyi model durumu kaydedilemedi.")
    ...
    LOGGER.info(
        "LSTM tamamlandi | symbol=%s | model=%s | class_weight=%s | model_path=%s | metrics_path=%s",
        symbol,
    ...
    LOGGER.info("LSTM validation metrics | %s", result["metrics"]["validation"])
    LOGGER.info("LSTM test metrics | %s", result["metrics"]["test"])
...
def main() -> None:
    ...
    for symbol in symbols:
        try:
            ...
            LOGGER.exception("LSTM egitimi basarisiz | symbol=%s | error=%s", symbol, exc)
            failures.append(symbol)

    if failures:
        raise RuntimeError(f"LSTM egitimi bazi semboller icin basarisiz oldu: {failures}")
